...
export function ProjectCard({
  ...
  return (
    <Card className="group gap-0 rounded-xl border-border bg-card shadow-soft transition-shadow duration-200 hover:shadow-lifted">
      <CardHeader className={cn("grid items-start gap-2 pb-3", isRtl ? "grid-cols-[auto_minmax(0,1fr)]" : "grid-cols-[minmax(0,1fr)_auto]")}>
        {isRtl && allowed && (
          <DropdownMenu>
        ...
        <div className="min-w-0">
          <Link
            ...
            className={cn("truncate text-base font-semibold tracking-tight transition-colors hover:text-primary block", isRtl && "text-right")}
          >
            {project.name}
          </Link>
          <p className={cn("mt-1 line-clamp-2 text-sm text-muted-foreground", isRtl && "text-right")}>
            {project.description}
          </p>
        </div>
        {!isRtl && allowed && (
      ...
      <CardFooter className={cn("flex items-center justify-between border-t border-border pt-4", isRtl && "flex-row-reverse")}>
